...
def sampling_gaussian(mu, logvar, device):
    sigma = torch.sqrt(torch.exp(logvar)).to(device)
...
class BaseVAE(BaseModel):
    def __init__(self, input_size, latent_dim, is_featExtract=False, device=torch.device('cuda')):
    ...
    def _infer_latent(self, mu, logvar):
        ...
        mu, logvar, z = sampling_gaussian(mu, logvar, self.device)
        return mu, logvar, z
